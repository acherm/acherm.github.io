...
Over the past weeks, I have been running exactly this experiment. The short take-away: coding agents can generate functional, UCI-compliant chess engines across a wide range of languages, some reaching over 2000 Elo.
...
**Below 1500 Elo (beginner to intermediate):** ChessRocq (Rocq/OCaml, Claude Code) at ~1450, TeXCCChess (LaTeX, Claude Code) at ~1280, SQLChess (SQL, Claude Code) at ~1120, and TeXCodex (LaTeX, Codex) at ~900. These engines are constrained by their languages' computational limitations but still play complete games.
...
Starting soon, I'll publish a **series of blog posts** (roughly one every three days) each diving deep into a specific engine. The first installment will cover **TeXCCChess**, the LaTeX chess engine, because frankly, a chess engine that runs inside `pdflatex` deserves its own post.
...
*Elo estimates are approximate and based on 50-game gauntlets against Stockfish with UCI_LimitStrength. Some results may be revised as evaluation continues. Data, game records (PGN), and analysis tools will be made available.*
